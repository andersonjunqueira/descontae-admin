...
class PlanoList extends Component {
    ...
    render() {
        ...
        const headers = [
            { classNames: 'table-xs-75 table-sm-85 table-md-85 table-lg-90', label: (<Intl str="titulo"></Intl>), orderField: 'titulo' },
            { classNames: 'table-xs-25 table-sm-15 table-md-15 table-lg-10 text-center' }
        ];

        return (
            <Form onSubmit={handleSubmit(doSubmit)}>
                <h4><Intl str='pesquisa-planos'></Intl></h4>

                <Row>
                    <Col xs={12} md={12}>
                        <Text name="titulo" label={<Intl str='titulo'></Intl>} maxLength={100}/>
                    </Col>
                </Row>
                ...
                <Link className="btn btn-secondary" to="/planos/novo">
                    <i className="fa fa-plus"></i> <Intl str='novo-plano' className="hidden-xs-down"></Intl>
                </Link>
                        ...
                        {Object.keys(data.content).map(function(key) {
                            ...
                            return (<tr key={key}>
                                ...
                                <td className="text-right">
                                    <Link className="btn btn-secondary btn-sm" to={`/planos/${r.id}`}>
